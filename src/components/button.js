...
const Button = ({ children, className, elevated }) => {
  return (
    <button className={classNames('px-4 relative py-3 text-blue-100 text-base font-plex-hebrew bg-blue-400 flex items-center justify-between md-max:text-sm', className)}>
      {children}
      <ArrowRight className='md-max:w-4 md-max:h-4'/>
      {
        elevated && (
